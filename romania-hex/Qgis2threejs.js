...
Q3D.Config = {
  ...
  viewpoint: {
    // z-up
    default: {
      // assumed that origin is (0, 0, 0) and base extent width in 3D world coordinates is 1
      pos: new THREE.Vector3(0, -0.5, 0.08),
...
(function () {
  ...
  app.init = function (container) {
    ...
    if (params.anisotropy)
      conf.texture.anisotropy = parseFloat(params.anisotropy);

    if (params.cx !== undefined)
      conf.viewpoint.pos = new THREE.Vector3(
        parseFloat(params.cx),
        parseFloat(params.cy),
        parseFloat(params.cz)
      );
    if (params.tx !== undefined)
      conf.viewpoint.lookAt = new THREE.Vector3(
    ...
    app.scene.addEventListener('cameraUpdateRequest', function (event) {
      ...
      if (app.controls.target !== undefined)
        app.controls.target.copy(event.focal);
    ...
    app.queryMarker = new THREE.Mesh(
      new THREE.SphereBufferGeometry(opt.radius, 32, 32),
      new THREE.MeshLambertMaterial({
        color: opt.color,
        opacity: opt.opacity,
        transparent: opt.opacity < 1,
      })
    );
    app.queryMarker.name = 'marker';

    app.queryMarker.onBeforeRender = function (
      renderer,
      scene,
      camera,
      geometry,
      ...
      this.scale.setScalar(
        this.position.distanceTo(camera.position) *
          (camera.isPerspectiveCamera ? 1 : conf.qmarker.k)
      );
  ...
  app.loadJSONObject = function (jsonObject) {
    ...
    if (jsonObject.animation !== undefined)
      app.animation.keyframes.load(jsonObject.animation.groups);
  };
  ...
  app.adjustCameraPosition = function (force) {
    ...
    app.cameraAction.zoom(
      ...
      vec3.y,
      (bbox.max.z + vec3.z) / 2,
      app.scene.userData.baseExtent.width
    );
  };
  ...
  app.buildViewHelper = function (container) {
    ...
    if (app.viewHelper !== undefined) {
      app.viewHelper.removeEventListener(
        'requestAnimation',
        app.startViewHelperAnimation
      );
    }
    ...
    app.viewHelper.addEventListener(
      'requestAnimation',
      app.startViewHelperAnimation
    );
  };
    ...
    keyframes: {
      ...
      start: function () {
        ...
        this.keyframeGroups.forEach(function (group) {
          ...
          var layer =
            group.layerId !== undefined
              ? app.scene.mapLayers[group.layerId]
              : undefined;
          ...
          for (var i = 0; i < keyframes.length - 1; i++) {
            t2 = new TWEEN.Tween(group.prop_list[i])
              .delay(keyframes[i].delay)
              .onStart(onStart)
              .to(group.prop_list[i + 1], keyframes[i].duration)
              .onComplete(onComplete);

            if (keyframes[i].easing) {
              t2.easing(_this.easingFunction(keyframes[i].easing)).onUpdate(
                group.onUpdate
              );
            }
  ...
  app.highlightFeature = function (object) {
    ...
    if (
      !layer ||
      layer.type == Q3D.LayerType.DEM ||
      layer.type == Q3D.LayerType.PointCloud
    )
      return;
    ...
    app.measure = {
      ...
      start: function () {
        ...
        if (!this.geom) {
          ...
          this.mtl = new THREE.MeshLambertMaterial({
            color: opt.color,
            opacity: opt.opacity,
            transparent: opt.opacity < 1,
          });
      ...
      addPoint: function (pt) {
        ...
        if (path.length > 1) {
          // add a line
          var v = path[path.length - 2]
              .toArray()
              .concat(path[path.length - 1].toArray()),
            geom = new THREE.BufferGeometry().setAttribute(
              'position',
      ...
      showResult: function () {
        ...
        html +=
          ...
          this.formatLength(total) +
          ' m</td><td></td></tr>';
        html +=
          '<tr><td>Horizontal distance:</td><td>' +
          this.formatLength(totalxy) +
          ' m</td><td></td></tr>';
        html +=
          '<tr><td>Vertical difference:</td><td>' +
          this.formatLength(dz) +
          ' m</td><td><span class="tooltip tooltip-btn" data-tooltip="elevation difference between start point and end point">?</span></td></tr>';
        html += '</table>';
...
(function () {
  ...
  gui.init = function () {
    ...
    ON_CLICK('infobtn', function () {
      ...
      if (gui.popup.isVisible() && gui.popup.content == 'pageinfo')
        gui.popup.hide();
      else gui.showInfo();
    });
    ...
    if (e) {
      if (show_coords) {
        ...
        if (conf.debugMode) {
          ...
          e.innerHTML +=
            '<br>WLD: ' +
            [point.x.toFixed(8), point.y.toFixed(8), point.z.toFixed(8)].join(
              ', '
            );
          e.innerHTML +=
            '<br><br>ORG: ' +
            [
              p.origin.x.toFixed(8),
              p.origin.y.toFixed(8),
              p.origin.z.toFixed(8),
            ].join(', ');
          e.innerHTML +=
            '<br>BE CNTR: ' + [be.cx.toFixed(8), be.cy.toFixed(8)].join(', ');
          e.innerHTML +=
            '<br>BE SIZE: ' +
            [be.width.toFixed(8), be.height.toFixed(8)].join(', ');
  ...
  gui.layerPanel = {
    init: function () {
      ...
      Object.keys(app.scene.mapLayers).forEach(function (layerId) {
        ...
        e.querySelector('input[type=checkbox]').addEventListener(
          'change',
          function () {
  ...
  loadJSONObject: function (jsonObject, callback) {
    ...
    if (m.bm) {
      ...
      var updateAspect = (this._listener = function () {
        mtl.resolution = new THREE.Vector2(
          Q3D.application.width,
          Q3D.application.height
        );
      });
...
Q3D.Scene.prototype.loadJSONObject = function (jsonObject) {
  if (jsonObject.type == 'scene') {
    ...
    if (p !== undefined) {
      ...
      // light
      var rotation0 = this.userData.baseExtent
        ? this.userData.baseExtent.rotation
        : 0;
      if (
        p.light != this.userData.light ||
        p.baseExtent.rotation != rotation0
      ) {
        this.lightGroup.clear();
        this.buildLights(
          Q3D.Config.lights[p.light] || Q3D.Config.lights.directional,
          p.baseExtent.rotation
        );
        ...
        if (v.pos === undefined) {
          ...
          if (be.rotation) {
            v = {
              pos: v.pos
                .clone()
                .applyAxisAngle(Q3D.uv.k, be.rotation * Q3D.deg2rad),
              lookAt: v.lookAt
                .clone()
                .applyAxisAngle(Q3D.uv.k, be.rotation * Q3D.deg2rad),
            };
...
Q3D.DEMBlock.prototype = {
  ...
  loadJSONObject: function (obj, layer, callback) {
    ...
    var mesh = new THREE.Mesh(
      geom,
      (this.materials[this.currentMtlIndex] || {}).mtl
    );
  ...
  addEdges: function (layer, parent, material, z0) {
    ...
    vl.forEach(function (v) {
      var geom = new THREE.BufferGeometry().setAttribute(
        'position',
        new THREE.Float32BufferAttribute(v, 3)
      );
  ...
  buildSides: function (layer, parent, material, z0) {
    ...
    for (var i = 0, l = polygons.length; i < l; i++) {
      ...
      for (j = 1, m = bnds.length; j < m; j++) {
        shape.holes.push(
          new THREE.Path(Q3D.Utils.flatArrayToVec2Array(bnds[j], 3))
        );
      }
  ...
  if (jsonObject.type == 'layer') {
    ...
    if (jsonObject.properties.clipped) {
      ...
      if (rotation) {
        ...
        this.objectGroup.position.applyAxisAngle(
          Q3D.uv.k,
          rotation * Q3D.deg2rad
        );
...
Q3D.DEMLayer.prototype.buildBlock = function (jsonObject, scene, layer) {
  ...
  if (block === undefined) {
    block = layer.properties.clipped
  ...
  block.loadJSONObject(jsonObject, this, function (mesh) {
    ...
    if (jsonObject.edges) {
      ...
      block.addEdges(
        ...
        mesh,
        material.mtl,
        jsonObject.sides ? jsonObject.sides.bottom : undefined
      );
    }
...
Q3D.DEMLayer.prototype.prepareTexAnimation = function (from, to) {
  ...
  for (var i = 0; i < this.blocks.length; i++) {
    ...
    canvas.width =
      img_from.width > img_to.width ? img_from.width : img_to.width;
    canvas.height =
      img_from.width > img_to.width ? img_from.height : img_to.height;
    ...
    if (img_from instanceof ImageData) {
      // WebKit Bridge
...
Q3D.DEMLayer.prototype.setTextureAt = function (progress, effect) {
  ...
  for (var i = 0; i < this.anim.length; i++) {
    ...
    if (effect == 0) {
      // fade in
      a.ctx.globalAlpha = 1;
      a.ctx.drawImage(a.img_from, 0, 0, w0, h0, 0, 0, w, h);
      a.ctx.globalAlpha = progress;
      a.ctx.drawImage(a.img_to, 0, 0, w1, h1, 0, 0, w, h);
    } else if (effect == 2) {
      // slide to left (not used)
      if (progress === null) {
        a.ctx.drawImage(a.img_from, 0, 0, w0, h0, 0, 0, w, h);
      } else {
        ew1 = w1 * progress;
        ew = w * progress;
        a.ctx.drawImage(a.img_to, w1 - ew1, 0, ew1, h1, w - ew, 0, ew, h);
      }
    }
  ...
  for (var i = 0, l = features.length; i < l; i++) {
    ...
    getPointsFunc(f).forEach(function (pt) {
      ...
      if (hasOtl) {
        ...
        for (j = 0; j < 9; j++) {
          if (j != 4)
            ctx.fillText(text, x + Math.floor(j / 3) - 1, y + (j % 3) - 1);
        }
      }
      ...
      mtl = new THREE.SpriteMaterial({
        map: new THREE.TextureLoader().load(canvas.toDataURL(), function () {
          _this.requestRender();
        }),
        opacity: opacity,
        transparent: true,
      ...
      if (hasConn) {
        ...
        geom.setAttribute(
          'position',
          new THREE.Float32BufferAttribute(vec.toArray().concat(pt), 3)
        );
...
Q3D.PointLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  if (
    ...
    jsonObject.properties.objType == '3D Model' &&
...
Q3D.PointLayer.prototype.buildPoints = function (features, startIndex) {
  ...
  for (var fidx = 0; fidx < features.length; fidx++) {
    ...
    geom = new THREE.BufferGeometry().setAttribute(
      'position',
      new THREE.Float32BufferAttribute(f.geom.pts, 3)
    );
    mtl = this.materials.mtl(f.mtl);
...
Q3D.LineLayer.prototype.build = function (features, startIndex) {
  ...
  var createObject =
    this._createObject || this.createObjFunc(this.properties.objType);
...
Q3D.Tweens.cameraMotion = {
  ...
  init: function (group) {
    ...
    for (var i = 0; i < keyframes.length; i++) {
      ...
      if (i > 0) {
        dist_list.push(
          Math.sqrt((p.x - p0.x) * (p.x - p0.x) + (p.y - p0.y) * (p.y - p0.y))
        );
      }
      p0 = p;
    }
    ...
    group.onUpdate = function (obj, elapsed, is_first) {
      ...
      dz = c
        ? (1 - Math.pow(2 * p - 1, 2)) * dist_list[group.currentIndex] * c
        : 0;
